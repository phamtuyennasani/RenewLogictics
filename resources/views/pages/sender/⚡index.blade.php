...
new class extends Component {
    ...
    {
        return Member::with(['sale:id,fullname,code,username', 'ctv:id,fullname,username,code', 'province:id,name', 'ward:id,name'])
            ->sender()
            ->visibleTo(auth()->user())
    ...
    public function deleteSelected()
    {
        if (empty($this->xCheck)) {
            Flux::toast(duration: 2000, heading: 'Cảnh báo', text: 'Vui lòng chọn Sender cần xóa!', variant: 'warning');
            return;
        }
        ...
        $this->dispatch('open-confirm', [
            'title'   => 'Xác nhận xóa nhiều Sender',
            'message' => "Bạn có chắc chắn muốn xóa {$count} Sender đã chọn? Hành động này không thể hoàn tác.",
            'variant' => 'danger',
        ]);
    }
    ...
    public function deleteItem($id)
    {
        ...
        $this->dispatch('open-confirm', [
            'title'   => 'Xác nhận xóa',
            'message' => 'Bạn có chắc chắn muốn xóa Sender này? Hành động này không thể hoàn tác.',
            'variant' => 'danger',
        ]);
    }
    ...
    public function handleConfirmAction()
    {
        match ($this->pendingAction) {
            'deleteItem'     => Member::whereKey($this->pendingId)->sender()->visibleTo(auth()->user())->firstOrFail()->delete(),
            'deleteSelected' => Member::whereIn('id', $this->xCheck)->sender()->visibleTo(auth()->user())->delete(),
            default          => null,
        };
        ...
        Flux::toast(duration: 2000, heading: 'Thành công', text: 'Xóa Sender thành công!', variant: 'success');
    }
...
<div x-data="tableCheck" class="space-y-4" style="--gradient: linear-gradient(135deg, {{ $primaryHex }}, {{ $accentHex }});">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl font-bold text-neutral-900 capitalize mt-0.5">Danh sách Sender</h1>
        </div>
        <div class="flex items-center gap-3 shrink-0">
            ...
            <a href="{{ route('sender.add') }}" wire:navigate
        ...
        <div class="px-5 py-4 flex items-center justify-between border-b border-neutral-100">
            <span class="text-sm text-neutral-500 leading-8">
                @if ($this->items->total() > 0)
                    ...
                    của <span class="font-semibold text-neutral-700">{{ $this->items->total() }}</span> Sender
                @else
                    Không có Sender nào
                @endif
            </span>
                    ...
                    @empty
                        <tr>
                            <td colspan="20" class="px-5 py-16 text-center">
                                <p class="text-sm font-medium text-neutral-600">Chưa có Sender nào</p>
                                <a href="{{ route('sender.add') }}" wire:navigate class="mt-3 inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white rounded-xl transition-all shadow-sm hover:shadow-md" style="{{ $gradientStyle }}">Thêm mới</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
