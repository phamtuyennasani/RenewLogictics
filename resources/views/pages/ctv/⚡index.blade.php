...
new class extends Component {
    ...
    public function items()
    {
        ...
        // Lấy map city_id => name một lần để tránh N+1
        $cityIds = $users->pluck('options')->map(fn($opt) => data_get($opt, 'company.city_id'))->filter()->unique();
        $cityMap = DB::table('province')->whereIn('id', $cityIds)->pluck('name', 'id');

        // Gắn city_name vào từng user
        $users->getCollection()->transform(function ($user) use ($cityMap) {
            $cityId = data_get($user->options, 'company.city_id');
            $user->city_name = $cityId ? ($cityMap[$cityId] ?? null) : null;
            return $user;
        });
    ...
    public function deleteSelected()
    {
        ...
        $this->dispatch('open-confirm', [
            ...
            'message' => "Bạn có chắc chắn muốn xóa {$count} tài khoản CTV đã chọn? Hành động này không thể hoàn tác.",
            'variant' => 'danger',
        ]);
    }
    ...
    public function deleteItem($id)
    {
        ...
        $this->dispatch('open-confirm', [
            'title'   => 'Xác nhận xóa',
            'message' => 'Bạn có chắc chắn muốn xóa tài khoản CTV này? Hành động này không thể hoàn tác.',
            'variant' => 'danger',
        ]);
    }
    ...
    public function handleConfirmAction()
    {
        ...
        Flux::toast(duration: 2000, heading: 'Thành công', text: 'Xóa tài khoản CTV thành công!', variant: 'success');
    }
...
<div x-data="tableCheck" class="space-y-4" style="--gradient: linear-gradient(135deg, {{ $primaryHex }}, {{ $accentHex }});">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl font-bold text-neutral-900 capitalize mt-0.5">Danh sách tài khoản CTV</h1>
        </div>
        <div class="flex items-center gap-3 shrink-0">
            ...
            <a href="{{ route('ctv.add') }}" wire:navigate
        ...
        <div class="px-5 py-4 flex items-center justify-between border-b border-neutral-100">
            <span class="text-sm text-neutral-500 leading-8">
                @if ($this->items->total() > 0)
                    ...
                    của <span class="font-semibold text-neutral-700">{{ $this->items->total() }}</span> tài khoản CTV
                @else
                    Không có tài khoản CTV nào
                @endif
            </span>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-neutral-50 border-b border-neutral-100">
                        ...
                        <th class="px-4 py-3.5 text-xs font-semibold text-neutral-500 uppercase tracking-wide">Mã CTV</th>
                ...
                <tbody class="divide-y divide-neutral-100">
                    @forelse ($this->items as $v)
                        <tr class="hover:bg-neutral-50/60 transition-colors">
                            ...
                            <td class="px-4 py-3.5 text-sm font-medium text-neutral-900">{{ $v->username }}</td>
                            <td class="px-4 py-3.5"><span class="text-sm font-mono text-neutral-600 bg-neutral-100 px-2 py-0.5 rounded">{{ $v->code ?? '—' }}</span></td>
                            <td class="px-4 py-3.5">
                                <p class="text-sm font-medium text-neutral-700">{{ $v->fullname ?? '—' }}</p>
                                @if($v->sale)
                                    <p class="text-xs text-neutral-400 mt-0.5">
                                        {{ $v->sale->fullname ?: $v->sale->username }} - {{ $v->sale->code ?: '—' }}
                                    </p>
                                @endif
                            </td>
                            <td class="px-4 py-3.5">
                                <p class="text-sm font-medium text-neutral-700">
                                    {{ data_get($v->options, 'company.company_short_name') ?: data_get($v->options, 'company.company_name') ?: '—' }}
                                </p>
                                @if(data_get($v->options, 'company.address_detail') || $v->city_name)
                                    <p class="text-xs text-neutral-400 mt-0.5">
                                        {{ data_get($v->options, 'company.address_detail') }}
                                        @if($v->city_name)
                                            {{ data_get($v->options, 'company.address_detail') ? ', ' : '' }}{{ $v->city_name }}
                                        @endif
                                    </p>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="20" class="px-5 py-16 text-center">
                                <p class="text-sm font-medium text-neutral-600">Chưa có tài khoản CTV nào</p>
                                <a href="{{ route('ctv.add') }}" wire:navigate class="mt-3 inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white rounded-xl transition-all shadow-sm hover:shadow-md" style="{{ $gradientStyle }}">Thêm mới</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
