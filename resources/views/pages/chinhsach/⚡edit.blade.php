...
new class extends Component {
    ...
    public function mount($slug)
    {
        ...
        if (!preg_match('/^[a-z0-9-]+$/', $slug)) {
            abort(404, 'Chính sách không hợp lệ');
        }
    ...
    public function save()
    {
        ...
        if (strlen($this->contentvi) > 16777215) { // MEDIUMTEXT max length
            Flux::toast(
                duration: 2000,
                heading: 'Lỗi',
                text: 'Nội dung quá dài, vui lòng rút gọn',
                variant: 'danger'
            );
            return;
        }
        ...
        try {
            ...
            // Use updateOrInsert to avoid race condition
            DB::table('static')->updateOrInsert(
                ['type' => $this->slug],
            ...
            Flux::toast(
                duration: 2000,
                heading: 'Lỗi',
                text: 'Không thể lưu chính sách. Vui lòng thử lại.',
                variant: 'danger'
            );

            \Log::error('Policy save error: ' . $e->getMessage());
        }
    }
