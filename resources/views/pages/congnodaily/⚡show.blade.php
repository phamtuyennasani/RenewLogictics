...
new #[Layout('layouts.app')] #[Title('Chi tiết công nợ đại lý')] class extends Component {
    ...
    public function addPayment(): void
    {
        ...
        Flux::toast(heading: 'Đã ghi nhận thanh toán', text: 'Số tiền đã được cập nhật vào công nợ đại lý.', variant: 'success');
    }
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($debt->payments->sortByDesc('paid_at') as $payment)
                        <div class="rounded-xl border border-neutral-100 bg-neutral-50 px-3 py-2">
                            ...
                            @if ($payment->note)
                                <p class="mt-1 text-xs text-neutral-600">{{ $payment->note }}</p>
                            @endif
                        </div>
                    @empty
