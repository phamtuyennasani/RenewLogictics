...
>
    ...
    {{-- =============================================
         SCROLLABLE MENU
         ============================================= --}}
    <div class="flex-1 overflow-y-auto px-2 py-4 scrollbar-thin">
            ...
            <div class="space-y-0.5 mb-6">
                @foreach ($items as $item)
                    @php
                        ...
                        // Kiểm tra con nào đang active
                        $isChildActive = false;
                        if ($hasChildren) {
                            foreach ($item['children'] as $child) {
                                $childType = $child['route_params']['type'] ?? null;
                                if (request()->routeIs($child['route']) && request()->route('type') == $childType) {
                                    $isChildActive = true;
                                    break;
                                }
                            }
                        ...
                        // Parent xổ ra khi: con active HOẶC đã được expand trước đó
                        $isExpanded = $hasChildren && ($isChildActive || ($expandedGroups[$groupLabel] ?? false));

                        // Parent active (nền nhạt) khi: con đang active
                        $isParentActive = $isChildActive;
                    @endphp

                    @if ($hasChildren)
                        {{-- PARENT WITH CHILDREN (dropdown) --}}
                        <div x-data="{ open: {{ $isExpanded ? 'true' : 'false' }} }">
                            <a href="javascript:void(0)"
                               class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 group
                                      {{ $isParentActive ? 'bg-[var(--color-primary-50)] text-[var(--color-primary-600)] font-medium' : 'text-neutral-600 hover:bg-neutral-100 hover:text-neutral-900' }}"
                               @click="open = !open"
                            ...
                            <div x-show="open"
                                ...
                                @foreach ($item['children'] as $child)
                                    @php
                                        ...
                                        $isChildActive = request()->routeIs($child['route']) && request()->route('type') == ($routeParams['type'] ?? null);
                                    @endphp
                                    <a href="{{ route($child['route'], $routeParams) }}"
                                       class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors
                                              {{ $isChildActive ? 'text-[var(--color-primary-600)] font-semibold bg-[var(--color-primary-50)]' : 'text-neutral-500 hover:text-neutral-800 hover:bg-neutral-50' }}">
                                        <span class="w-1.5 h-1.5 rounded-full shrink-0 {{ $isChildActive ? 'bg-[var(--color-primary-500)]' : 'bg-neutral-300' }}"></span>
                                        {{ $child['label'] }}
                                    </a>
                                @endforeach
                            </div>
                    ...
                    @else
                        ...
                        <a href="{{ route($item['route']) }}"
                           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 group
                                  {{ request()->routeIs($item['route']) ? 'bg-[var(--color-primary-50)] text-[var(--color-primary-700)]' : 'text-neutral-600 hover:bg-neutral-100 hover:text-neutral-900' }}">
                            {{-- Icon --}}
                            <span class="w-5 h-5 flex items-center justify-center shrink-0">
                                @switch($item['icon'])
                                    @case('dashboard')
